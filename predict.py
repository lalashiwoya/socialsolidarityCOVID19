...
def ensemble_model_pred(model_path_ls, df):
    """
    ...
    for path in model_path_ls:
        ...
        if path.startswith('bert_cnn'):
            print('bert_cnn')
            embed_model = BertModel.from_pretrained(pretrained_weights)
            model = bertCNN(embed_model=embed_model, dropout=0.2, kernel_num=4, kernel_sizes=[3, 4, 5, 6], num_labels=args.num_labels)
        elif path.startswith('bert_dpcnn'):
            print('bert_dpcnn')
            embed_model = BertModel.from_pretrained(pretrained_weights)
            model = bertDPCNN(embed_model=embed_model, num_filters=100, num_labels=args.num_labels)
        else:
        ...
        data_loader = convert_data_into_features(df, tokenizer, max_len=MAX_LEN, batch_size=BATCH_SIZE, shuffle=False)
...
def run(model_names, read_dir, write_dir):
    ...
    for file in files:
        ...
        if len(model_names) == 1:
            predictions = ensemble_model_pred(model_names, df)
            print(len(df), len(predictions[0]))
            df['predict'] = predictions[0]
            df = df[['id', 'predict']]
        else:
            predictions, predictions_ensemble = ensemble_model_pred(model_names, df)
            ...
            df = df[['id', 'predict', 'predict_ensemble']]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_dir", type=str, required=True, help="Directory which saves models")
    parser.add_argument("--model_name", type=str, default=None, help="Name of the model, for single model prediction")
    parser.add_argument("--data_dir", type=str, default='twitter_data',
                        help="Directory which saves data to be predicted")
    parser.add_argument("--output_dir", type=str, default='predictions', help="Path to save the prediction results")
    ...
    run(model_path_ls, args.data_dir, args.output_dir)